...
## Addendum A2 — A telefon- és óra-`applicationId` egyezése (Data Layer kézbesítési előfeltétel)

A 7-bg-g on-device verifikáció előtt rögzítve: a Wearable Data Layer egy
`DataItem`-et az **adott app-hoz tart privátként**, és csak **ugyanannak az
appnak** kézbesíti a többi node-on — ahol az „ugyanaz az app" párosítás
`applicationId` + aláíró kulcs alapján dől el. Az aláíró kulcs nálunk már
egyező (közös debug keystore egy gépen — lásd ADR 0015 Következmények); a
hiányzó fél az `applicationId`-egyezés volt.

A skeleton-baseline (ADR 0015 D8) óta a watch `applicationId` a `flutter
create`-default `dev.csakos.sailingassistant.watch` maradt, miközben a phone-é
az ARCHITECTURE §1.5 szerinti `com.csakos.foretack`. Eltérő `applicationId`
mellett az óra a phone által írt `/race-state` `DataItem`-et **nem kapja meg**
(a `WatchHomeView` a loading-spinneren ragadna), az aláírás-egyezéstől
függetlenül.

### Döntés (A2)
A watch `applicationId`-t a phone-éhoz **igazítjuk** (`com.csakos.foretack`,
`apps/watch/android/app/build.gradle.kts`). Külön eszközökön (Pixel ↔ SM-R880)
az azonos `applicationId` ütközés nélküli — ez a standard Wear-companion minta,
és egyúttal behozza az órát a §1.5-ben már rögzített brandbe.

A `namespace` (a Kotlin forrás-package, `dev.csakos.sailingassistant.watch`)
**nem** mozdul: AGP 7+ óta a forrás-package független az install-identitástól, a
Data Layer párosítás csak az `applicationId`-t nézi. Így a `MainActivity.kt`
áthelyezése és a `wearable_bridge` plugin `com.csakos.foretack.wearable_bridge`
namespace-e érintetlen — minimális diff, YAGNI.

### Elvetett (A2)
- **Empirikus-először** (a 7-bg-g első körén próbálni, csak bukásnál igazítani):
  a platform-előfeltétel ismeretében ez majdnem biztosan elpazarolt on-device
  kör — az `applicationId`-eltérés a dokumentált blokkoló.
- **A `namespace` egyidejű átnevezése `com.csakos.foretack`-re:** a Data Layer
  szempontjából felesleges; csak forrás-package-mozgatási churn-t adna
  (`MainActivity.kt` áthelyezés) haszon nélkül.
